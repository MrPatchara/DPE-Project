...
def speak_text(text):
    """ ใช้ TTS (Text-to-Speech) พูดข้อความหลังจากเล่นเสียง Beep """
    def _speak():
        engine.say(text)
        engine.runAndWait()
...
def play_beep(beep_type="shuttle"):
    ...
    def _beep():
        ...
        while pygame.mixer.music.get_busy():  # รอให้เสียงเล่นจบก่อนพูด
            time.sleep(0.1)
...
def start_protocol():
    ...
    while running:
        ...
        with lock:
            for player in selected_players:
                if player_status[player] not in ["Disqualified"]:  
                    ...
                    passed_checkpoints[player] = {"A": False, "B": False}  # รีเซ็ตการผ่านเซ็นเซอร์
        ...
        # ✅ ตรวจสอบสถานะเมื่อหมด Shuttle
        with lock:
            for player in selected_players:
                if player_status[player] == "Disqualified":
                    continue  # ถ้า Disqualified แล้ว ข้ามเลย

                # ❌ ถ้าไม่ผ่านครบทั้ง A และ B → "Warning"
...
def reset_shuttle():
    """ รีเซ็ตสถานะเป็น Waiting เฉพาะคนที่ยังไม่ Disqualified """
    global current_shuttle, current_level, running
    with lock:
        current_shuttle += 1

        # ✅ เล่นเสียง Beep ก่อนประกาศ Shuttle
        play_beep("shuttle")
        ...
        if current_level >= len(protocol) or current_shuttle > protocol[current_level]["shuttles"]:
            ...
            # ✅ เล่นเสียง Double Beep ก่อนประกาศ Level
            play_beep("level")
            time.sleep(2)  # รอให้เสียง Double Beep จบก่อนพูด
