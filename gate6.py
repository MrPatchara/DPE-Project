...
def on_message(client, userdata, msg):
    global selected_athlete, running
    ...
    if total_selected == 1:
        ...
        if aid not in start_times:
            ...
            update_result_table()
            log(f"Single-sensor: Stop timing at {sensor_key} → Time: {duration:.2f}s")
        ...
        if len(player_sensors[aid]) == 0:
            ...
            results[aid].append(duration)
            update_result_table()
            log(f"Dual-sensor: Stop timing at {sensor_key} → Time: {duration:.2f}s")
            # reset
            player_sensors[aid] = []
            del start_times[aid]
...
def start_session():
    global running, session_started
    if not selected_athlete:
...
def reset_session():
    global running, start_times, results, session_started
    running = False
