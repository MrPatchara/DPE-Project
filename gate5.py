...
def on_message(client, userdata, msg):
    ...
    sensor_key = topic.split("_")[-1]  # ดึงตัวอักษร A–J จาก topic เช่น athlete_status_A
    if not active_sensors.get(sensor_key, False):
        return  # ข้ามถ้า sensor นี้ไม่ถูกเลือกใช้งาน
    ...
    if aid not in start_times:
        ...
        log(f"Start timing from sensor {sensor_key}...")
    else:
        # จบการจับเวลา
        end_time = time.time()
        ...
        update_result_table()
        log(f"End timing at sensor {sensor_key} → Time: {duration:.2f} seconds")
