...
def view_result_detail(tree, data):
    ...
    tk.Button(win, text="Plot Graphs", command=lambda: plot_graphs(times, distance), bg="navy", fg="white").pack(pady=5)
...
def plot_graphs(times, distance):
    ...
    fig.suptitle("Performance Graphs", fontsize=14)

    axs[0].plot(rounds, times, marker='o', color='blue')
    axs[0].set_title("Time per Round")
    axs[0].set_ylabel("Time (s)")

    axs[1].plot(rounds, speeds, marker='o', color='green')
    axs[1].set_title("Speed per Round")
    axs[1].set_ylabel("Speed (m/s)")

    axs[2].plot(rounds, accels, marker='o', color='red')
    axs[2].set_title("Acceleration per Round")
    ...
    # ✅ จัดการปิดกราฟแยกจาก root
    def close_graph_window():
        plt.close(fig)               # ปิด matplotlib figure
        graph_win.destroy()          # ปิด Tkinter window
