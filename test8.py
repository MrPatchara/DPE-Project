...
def start_protocol():
    ...
    while running:
        ...
        # ✅ รีเซ็ตเฉพาะคนที่ยังไม่ Fails
        with lock:
            for player in selected_players:
                if player_status[player] != "Fails":  
        ...
        with lock:
            for player in selected_players:
                if player_status[player] == "Fails":
                    continue  # ถ้า Fails ไปแล้ว ข้ามเลย

                # ❌ ถ้าไม่ผ่านครบทั้ง A และ B → "Fails"
                if passed_checkpoints[player]["A"] and passed_checkpoints[player]["B"]:
                    player_status[player] = "Passed"
                else:
                    player_status[player] = "Fails"  # ค้างไว้ ไม่ต้องรีเซ็ตอีก

        root.after(0, update_table)
        reset_shuttle()

def reset_shuttle():
    """ รีเซ็ตสถานะเป็น Waiting เฉพาะคนที่ยังไม่ Fails """
    ...
    with lock:
        ...
        # ✅ รีเซ็ตเฉพาะคนที่ยังไม่ Fails
        for player in selected_players:
            if player_status[player] != "Fails":  # ถ้าเป็น Fails แล้วไม่ต้องรีเซ็ต
