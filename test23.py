...
def check_yo_yo_shuttle_completion():
    ...
    with lock:
        for player in selected_players:
            ...
            # ✅ เงื่อนไข 1: ถ้าผู้เล่นผ่านทั้ง A → B → A (เวลา A ต้องมากกว่า B)
            if passed_A and passed_B and time_A > time_B:
                player_status[player] = "Passed"

            # ✅ เงื่อนไข 2: ถ้าผ่านแค่ A → B หรือ B → A แต่ไม่ครบ ให้ Warning
            elif passed_A or passed_B:
                warning_count[player] += 1
                if warning_count[player] >= 2:
                    player_status[player] = "Disqualified"  # ✅ ถ้า Warning ≥ 2 ตัดสิทธิ์
                else:
                    player_status[player] = "Warning"  # ✅ ให้ Warning

            # ✅ รีเซ็ตค่าการผ่าน Shuttle เพื่อใช้รอบถัดไป
            passed_checkpoints[player] = {"A": False, "B": False}

    root.after(0, update_table)
    root.after(500, rest_before_next_yo_yo_shuttle)  # ✅ รอ 0.5 วินาทีก่อนพัก 10 วิ
    ...
    for player, status in player_status.items():
        if selected_players[player]:
            ...
            if is_yo_yo_test:
                ...
                # บันทึกผลลัพธ์เมื่อผู้เล่น Fails หรือ Disqualified
...
def reset_test():
    ...
    with lock:
        for player in selected_players:
            ...
            # ✅ แยกการรีเซ็ตผลลัพธ์สำหรับ Beep และ Yo-Yo
            if is_yo_yo_test:
