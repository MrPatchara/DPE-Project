...
def on_message(client, userdata, msg):
    ...
    if total_selected == 1:
        ...
        if len(player_splits[aid]) == 1:
            start_times[aid] = now  # สำหรับป้ายแสดงเวลา
        ...
        if len(player_splits[aid]) == total_selected:
            ...
            if is_team_mode:
                if aid not in team_test_results:
                    team_test_results[aid] = []
                team_test_results[aid].append(total_time)
...
def update_result_table():
    ...
    if is_team_mode and selected_team:
        for aid in selected_team["members"]:
            ...
            name = f"{athlete['first_name']} {athlete['last_name']}" if athlete else aid
            for t in times:
                result_tree.insert("", "end", values=(name, f"{t:.6f} sec"))
...
def export_pdf(record):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmpfile:
        with PdfPages(tmpfile.name) as pdf:
            ...
            # หน้าแรก (Cover Page)
            fig_cover = plt.figure(figsize=(8.3, 11.7))  # A4 size
            plt.axis('off')
           ...
           # หน้าสอง: ตารางข้อมูล + Summary (รวมในหน้าเดียวอย่างเป็นทางการ)
            import pandas as pd
            ...
            table = ax.table(cellText=df.values,
                 ...
                 colLoc='center',
                 loc='upper center',
                 bbox=[0, 0.4, 1, 0.55])  # ✅ ขยับขึ้นให้พอดี
            ...
            summary_data = [
                ...
                ["Total Rounds", len(times)],
                ["Average Time (s)", f"{sum(times)/len(times):.6f}"],
                ["Average Speed (m/s)", f"{sum(speeds)/len(speeds):.6f}"],
                ["Average Acceleration (m/s²)", f"{sum(accels)/len(accels):.6f}"],
                ["Max Speed (m/s)", f"{max(speeds):.6f}"],
                ["Max Acceleration (m/s²)", f"{max(accels):.6f}"],
                ["Min Time (s)", f"{min(times):.6f}"]
            ]
            ...
            def add_chart(title, y_data, y_label, color):
                ...
                ax.plot(rounds, y_data, marker='o', color=color, linewidth=2)
            ...
            add_chart("Speed per Round", speeds, "Speed (m/s)", "green")
            add_chart("Acceleration per Round", accels, "Acceleration (m/s²)", "red")
