        ...
        with lock:
            for player in selected_players:
                if selected_players[player]:
                    if not passed_checkpoints[player]["B"]:  # ถ้ายังไม่ผ่านจุดที่ 2 เมื่อเวลาหมด
    ...
    if sensor_data in sensor_mapping:
        ...
        if selected_players[player]:
            with lock:
                ...
                if checkpoint == "A":
                    if not passed_checkpoints[player]["B"]:  
                        passed_checkpoints[player]["A"] = True  # ผ่าน A ครั้งแรก
                    else:
                        passed_checkpoints[player]["A"] = True  
                        passed_checkpoints[player]["B"] = False  # ผ่าน A หลังจาก B -> ครบรอบ
                        player_status[player] = "Passed"

                elif checkpoint == "B":
                    if not passed_checkpoints[player]["A"]:  
                        passed_checkpoints[player]["B"] = True  # ผ่าน B ครั้งแรก
                    else:
                        passed_checkpoints[player]["B"] = True  
                        passed_checkpoints[player]["A"] = False  # ผ่าน B หลังจาก A -> ครบรอบ
                        player_status[player] = "Passed"
