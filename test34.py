...
def finish_test_and_save():
    ...
    test_type = "Yo-Yo Test" if is_yo_yo_test else "Beep Test"
...
def view_test_history():
    ...
    for i, r in enumerate(results):
        ...
        try:
            ...
            elif "Yo-Yo Intermittent Recovery Test Level 1" in test_type:
                distance = get_yoyo_distance(shuttle)
                vo2max = get_yoyo_ir1_vo2max(distance)

            elif "Yo-Yo Intermittent Recovery Test Level 2" in test_type:
                distance = get_yoyo_distance(shuttle)
    ...
    delete_button = tk.Button(win, text="Delete Selected Result", command=delete_selected_result, bg="red", fg="white", font=("Arial", 11))
...
def get_yoyo_distance(shuttle):
    return shuttle * 40  # ไป-กลับ = 40 เมตร
...
finish_button = tk.Button(control_frame, text="Finished", font=FONT_MEDIUM, command=finish_test_and_save, width=12, bg="blue", fg="white")
...
athlete_menu.add_command(label="Manage_Athletes", command=manage_athletes_window)
