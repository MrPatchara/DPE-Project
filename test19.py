...
protocols = {
    ...
    "Yo-Yo Intermittent Recovery Test Level 1": [
        {"level": 1, "shuttles": 5, "time_per_shuttle": 30},
        {"level": 2, "shuttles": 6, "time_per_shuttle": 9},
        {"level": 3, "shuttles": 7, "time_per_shuttle": 8},
        {"level": 4, "shuttles": 8, "time_per_shuttle": 7},
    ],
}

def set_protocol():
    """ เปลี่ยนโปรโตคอลตามที่ผู้ใช้เลือก และเปิดปุ่ม Start Test """
    global protocol
    selected = selected_protocol.get()
    protocol = protocols[selected]  # ✅ อัปเดตโปรโตคอลที่ใช้งาน
    speak_text(f"Protocol set to {selected}")
    
    # ✅ เปิดปุ่ม Start Test หลังจากเลือกโปรโตคอลแล้ว
...
def on_message(client, userdata, msg):
    ...
    # ✅ ปรับปรุงโครงสร้างของ Sensor Mapping
    sensor_mapping = {}
    for i in range(10):  # รองรับ Player 1-10
        player = f"Player {i+1}"
        sensor_mapping[f"athlete_status_{chr(65 + i * 2)}"] = (player, "A")  # จุด A ของแต่ละ Player
        sensor_mapping[f"athlete_status_{chr(66 + i * 2)}"] = (player, "B")  # จุด B ของแต่ละ Player
    ...
    if sensor_data in sensor_mapping:
        ...
        if selected_players[player]:
            with lock:
                ...
                # ✅ ถ้าผ่านครบ A และ B ให้เปลี่ยนเป็น "Passed"
                if passed_checkpoints[player]["A"] and passed_checkpoints[player]["B"]:
                    player_status[player] = "Passed"
...
def update_table():
    ...
    for player, status in player_status.items():
        if selected_players[player]:
            ...
            # ✅ บันทึก Level - Shuttle สุดท้ายที่ Player Fails
...
# ✅ สร้างเมนู Dropdown ให้เลือกโปรโตคอล
protocol_menu = tk.OptionMenu(root, selected_protocol, *protocols.keys())
protocol_menu.pack()
